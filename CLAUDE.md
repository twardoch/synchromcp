# synchromcp - Project-Specific Guidelines

## Project Scope

**One sentence:** A Python CLI tool that synchronizes MCP server configurations between AI apps and machines.

## Key Files

- `PLAN.md` - Architecture, config locations, unified schema
- `TODO.md` - Flat task list with status markers
- `src/synchromcp/` - Main package source
...
|---------|---------|
| fire | CLI framework |
| pydantic | Data validation and mcpServers models |
...
1. **Config locations vary by app and OS** - See PLAN.md Section 1 for complete list
2. **mcpServers vs mcp_servers** - JSON uses camelCase, TOML uses snake_case
3. **Preserve other data** - When syncing, only modify the mcpServers section
4. **Claude Desktop is default source** - Other apps are sync targets
...
# Run CLI
python -m synchromcp list
python -m synchromcp sync --dry-run
```

## Testing Focus

- Test config discovery on each platform
- Test JSON reader preserves non-mcpServers data
- Test TOML key conversion (mcpServers <-> mcp_servers)
- Test --mounts with external volume paths
